...
class Crt extends ManagedComponent implements ToolTipMaker, AttributeListener {
  /**
   ...
   */
  public static final String _ID = "RGB CRT";
  ...
  public static final Attribute<String> BLINK_OPTION =
      Attributes.forOption("cursor", S.getter("rgbCrtCursor"), BLINK_OPTIONS);
  public static final Attribute<String> COLOR_OPTION =
      Attributes.forOption("color", S.getter("rgbCrtColor"), COLOR_OPTIONS);
  public static final Attribute<Integer> WIDTH_OPTION =
      Attributes.forInteger("width", S.getter("rgbCrtWidth"));
  public static final Attribute<Integer> HEIGHT_OPTION =
      Attributes.forInteger("height", S.getter("rgbCrtHeight"));
  public static final Attribute<Integer> SCALE_OPTION =
      Attributes.forIntegerRange("scale", S.getter("rgbCrtScale"), 1, 8);
  public static final Attribute<Integer> NCROP_OPTION =
      Attributes.forInteger("nCrop", S.getter("rgbCrtNCrop"));
  public static final Attribute<Integer> ECROP_OPTION =
      Attributes.forInteger("eCrop", S.getter("rgbCrtECrop"));
  public static final Attribute<Integer> SCROP_OPTION =
      Attributes.forInteger("sCrop", S.getter("rgbCrtSCrop"));
  public static final Attribute<Integer> WCROP_OPTION =
      Attributes.forInteger("wCrop", S.getter("rgbCrtWCrop"));

  private static final Attribute<?>[] ATTRIBUTES = {
    BLINK_OPTION, COLOR_OPTION, WIDTH_OPTION, HEIGHT_OPTION, SCALE_OPTION,
    NCROP_OPTION, ECROP_OPTION, SCROP_OPTION, WCROP_OPTION
  };
  ...
  private static class Factory extends AbstractComponentFactory {
    ...
    public Component createComponent(Location loc, AttributeSet attrs) {
      return new Crt(loc, attrs);
    }

    @Override
    public Bounds getOffsetBounds(AttributeSet attrs) {
      final var s = attrs.getValue(SCALE_OPTION);
      final var w = attrs.getValue(WIDTH_OPTION) - attrs.getValue(WCROP_OPTION) - attrs.getValue(ECROP_OPTION);
      final var h = attrs.getValue(HEIGHT_OPTION) - attrs.getValue(NCROP_OPTION) - attrs.getValue(SCROP_OPTION);
    ...
    public void paintIcon(ComponentDrawContext context, int x, int y, AttributeSet attrs) {
      drawCrtIcon(context, x, y);
    }
  }

  static final int P_CLK = 0;
  static final int P_HSYNC = 1;
  static final int P_VSYNC = 2;
  static final int P_DATA = 3;

  private Crt(Location loc, AttributeSet attrs) {
    super(loc, attrs, 5);
    setEnd(P_CLK, getLocation().translate(0, 0), BitWidth.ONE, EndData.INPUT_ONLY);
    setEnd(P_HSYNC, getLocation().translate(20, 0), BitWidth.ONE, EndData.INPUT_ONLY);
    setEnd(P_VSYNC, getLocation().translate(30, 0), BitWidth.ONE, EndData.INPUT_ONLY);
    configureComponent();
  ...
  public void propagate(CircuitState circuitState) {
    ...
    final var x = state.lastX;
    final var y = state.lastY;
    final var hsync = val(circuitState, P_HSYNC);
    final var vsync = val(circuitState, P_VSYNC);
    final var color = addr(circuitState, P_DATA);
    state.color = color;

    final var cm = getColorModel(attrs.getValue(COLOR_OPTION));

    if (state.tick(val(circuitState, P_CLK))) {
      final var g = state.img.getGraphics();
      g.setColor(new Color(cm.getRGB(color)));
      g.fillRect(x, y, 1, 1);
      if (state.lastHSync == Value.FALSE && hsync == Value.TRUE) {
        state.lastX = 0;
        state.lastY++;
      } else if (state.lastVSync == Value.FALSE && vsync == Value.TRUE) {
        state.lastY = 0;
      } else {
        state.lastX++;
      }
      state.lastHSync = hsync;
      state.lastVSync = vsync;
  ...
  public void draw(ComponentDrawContext context) {
    ...
    drawCrt(context, loc.getX(), loc.getY(), s);
  }

  static void drawCrtIcon(ComponentDrawContext context, int x, int y) {
  ...
  void drawCrt(ComponentDrawContext context, int x, int y, State state) {
    ...
    final var s = attrs.getValue(SCALE_OPTION);
    final var nc = attrs.getValue(NCROP_OPTION);
    final var ec = attrs.getValue(ECROP_OPTION);
    final var sc = attrs.getValue(SCROP_OPTION);
    final var wc = attrs.getValue(WCROP_OPTION);
    final var w = attrs.getValue(WIDTH_OPTION) - wc - ec;
    final var h = attrs.getValue(HEIGHT_OPTION) - nc - sc;
    final var bw = (Math.max(s * w + 14, 100));
    final var bh = (Math.max(s * h + 14, 20));
    ...
    g.drawImage(state.img, x + 7, y + 7, x + 7 + s * w, y + 7 + s * h, wc, nc, wc + w, nc + h, null);
  ...
  private State getState(CircuitState circuitState) {
    ...
    if (state == null) {
      ...
      state = new State(new BufferedImage(attrs.getValue(WIDTH_OPTION), attrs.getValue(HEIGHT_OPTION), BufferedImage.TYPE_INT_ARGB));
  ...
  private static class State implements ComponentState, Cloneable {
    ...
    public Value lastHSync;
    public Value lastVSync;
  ...
  public String getToolTip(ComponentUserEvent e) {
    ...
    switch (end) {
      case P_CLK:
        return S.get("rgbCrtCLK");
      case P_HSYNC:
        return S.get("rgbCrtHSync");
      case P_VSYNC:
        return S.get("rgbCrtVSync");
      case P_DATA:
        AttributeSet attrs = getAttributeSet();
        return S.get("rgbCrtData", attrs.getValue(COLOR_OPTION));
      default:
        return null;
    }
  }
  ...
  void configureComponent() {
    ...
    setEnd(P_DATA, getLocation().translate(40, 0), BitWidth.create(bpp), EndData.INPUT_ONLY);
    recomputeBounds();
